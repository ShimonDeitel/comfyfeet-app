...
final class Store: ObservableObject {
    @Published private(set) var items: [NightLog] = []
    ...
    private let fileName = "comfyfeet_items.json"
    ...
    private func load() {
        ...
        if let decoded = try? JSONDecoder().decode([NightLog].self, from: data) {
            items = decoded
        }
    }
